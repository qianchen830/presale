...
// 用法: cd ~/.openclaw/workspace/webdev-projects/presale && node import-users.js
const fs = require('fs');
...
async function main() {
  ...
  // 1. 加载所有 state JSON 文件
  const allStates = [];
  for (const name of USERS) {
    const fp = path.join(STATES_DIR, name + '.json');
    if (!fs.existsSync(fp)) { console.log('⚠️  找不到:', name); continue; }
    const state = JSON.parse(fs.readFileSync(fp, 'utf8'));
    allStates.push(state);
    console.log(`✅ ${name}: ${state.applications ? state.applications.length : 0}条申请`);
  }
  console.log(`\n📊 共读取 ${allStates.length} 个用户数据\n`);
  ...
  console.log('📦 合并后统计:');
  console.log(`  申请记录: ${merged.applications.length}`);
  console.log(`  跟进记录: ${merged.followUps.length}`);
  console.log(`  合同记录: ${merged.contracts.length}`);
  console.log(`  员工数据: ${merged.employees.length}`);
  console.log(`  部门数据: ${merged.departments.length}`);
  ...
  db.run(`CREATE TABLE IF NOT EXISTS app_state_history (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    data TEXT NOT NULL,
    created_at TEXT NOT NULL
  )`);
  ...
  for (const name of USERS) {
    ...
    db.run(
      ...
      [name, hash, name, 'user', '解决方案与项目经理部', now]
    );
  ...
  if (existingState.length && existingState[0].values.length) {
    ...
    if (old.length && old[0].values.length) {
      db.run('INSERT INTO app_state_history (data, created_at) VALUES (?, ?)', [old[0].values[0][0], old[0].values[0][1]]);
    }
    db.run('UPDATE app_state SET data=?, updated_at=? WHERE id=1', [stateJson, now]);
  } else {
    db.run('INSERT INTO app_state (id, data, updated_at) VALUES (1, ?, ?)', [stateJson, now]);
  }
  ...
  // 7. 验证
  const vDb = new SQL.Database(Buffer.from(fs.readFileSync(DB_PATH)));
  ...
  console.log('\n📋 数据库用户:');
  if (vUsers.length) {
    vUsers[0].values.forEach(r => console.log(`  - ${r[0]} (${r[1]})`));
  }
  ...
  console.log('\n💡 服务需重启才能加载新用户: fuser -k 3210/tcp && cd', path.dirname(DB_PATH), '&& nohup node server.js > /tmp/presale.log 2>&1 &');
}
